...
func TestSyncOrphanPod(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var orphanPod *v1.Pod
			if tc.podSelector != nil {
				podBuilder := buildPod().name(name).labels(tc.podSelector.MatchLabels).deletionTimestamp().trackingFinalizer()
				if tc.owner != nil {
				...
				if err != nil {
					t.Fatalf("Creating orphan pod with label selector: %v", err)
				}

				// Simulate syncing using pod selector
				selector, err := metav1.LabelSelectorAsSelector(tc.podSelector)
				if err != nil {
					t.Fatalf("Error parsing pod label selector: %v", err)
				}
				// Ensure cache is synced before calling syncOrphanPod
				if !cache.WaitForCacheSync(ctx.Done(), podInformer.HasSynced) {
					t.Fatalf("Failed to wait for pod informer cache sync")
				}
				// Sync orphan pod by selector
				err = manager.syncOrphanPod(ctx, selector.String())
				if err != nil {
					t.Fatalf("Failed to sync orphan pod by label selector: %v", err)
				}
			} else {
				podBuilder := buildPod().name(name).deletionTimestamp().trackingFinalizer()
				if tc.owner != nil {
